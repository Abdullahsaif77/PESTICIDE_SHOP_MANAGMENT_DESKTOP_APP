...
import { Users, DollarSign, Eye, ArrowLeft, X } from 'lucide-react';
import { ReportFilters } from './ReportFilters';
import { ReportTable } from './ReportTable';

const api = window.api || {};

export default function CustomerLedgerReport({ onBack }) {
  ...
  const columns = [
    ...
    { key: 'total_purchases', label: 'Total Purchases', sortable: true, render: (v) => `₨${(v || 0).toFixed(2)}` },
    { key: 'total_paid', label: 'Total Paid', sortable: true, render: (v) => `₨${(v || 0).toFixed(2)}` },
    { key: 'outstanding', label: 'Outstanding', sortable: true, render: (v) => `₨${(v || 0).toFixed(2)}` },
    { key: 'opening_balance', label: 'Opening Balance', sortable: true, render: (v) => `₨${(v || 0).toFixed(2)}` },
    { key: 'closing_balance', label: 'Closing Balance', sortable: true, render: (v) => `₨${(v || 0).toFixed(2)}` },
    {
      key: 'actions',
      label: 'Actions',
      render: (v, row) => (
...
const CustomerDetailModal = ({ customer, onClose }) => {
  ...
  const fetchLedgerDetails = async () => {
    setLoading(true);
    try {
      const result = await api.getCustomerLedgerDetails(customer.id);
      if (result && result.success) {
                ...
                <tbody className="divide-y divide-slate-100">
                  {ledgerEntries.map((entry, index) => (
                    <tr key={index} className="hover:bg-slate-50/70 transition-colors">
                      <td className="px-4 py-2 text-sm text-slate-700">{entry.invoice_number || '-'}</td>
