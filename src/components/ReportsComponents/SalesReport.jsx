...
import {
  ...
  Users, Percent, Calculator, Printer, Download,
  FileText, FileSpreadsheet, Calendar, ChevronLeft, ChevronRight,CreditCard,Box,
} from 'lucide-react';
import {
...
export default function SalesReport() {
  ...
  const columns = [
    ...
    { key: 'paid', label: 'Paid', sortable: true, render: (v) => `₨${v.toFixed(2)}` },
    { key: 'due', label: 'Due', sortable: true, render: (v) => `₨${v.toFixed(2)}` },
    { key: 'profit', label: 'Profit', sortable: true, render: (v) => `₨${v.toFixed(2)}` }
  ];
    ...
    <div className="p-4 sm:p-6 bg-gradient-to-br from-slate-50 via-white to-slate-100/50 min-h-screen">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        ...
        <SummaryCard
          icon={CreditCard}
          label="Credit Sales"
          value={`₨${(summary.creditSales || 0).toFixed(2)}`}
        ...
        <SummaryCard
          ...
          value={`₨${(summary.discount || 0).toFixed(2)}`}
          color="text-rose-600"
          bgColor="bg-rose-50"
        />
        <SummaryCard
